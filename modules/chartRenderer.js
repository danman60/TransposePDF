...
class ChartRenderer {
  ...
  renderPlannedStructuredContent(song, options = {}) {
    ...
    const pageHtml = plan.pages.map((page, pageIndex) => {
      ...
      const innerPageWidth = plan.spec.pageWidth - plan.spec.margins.left - plan.spec.margins.right;
      return `<section class="chart-page${song.layout?.layoutMode ? ' layout-mode' : ''}" data-chart-page="${pageIndex + 1}" style="--page-columns:${page.spanRows ? 1 : plan.spec.columns};--column-template:${page.spanRows ? '1fr' : plan.spec.columnRatios.map(value => `${value}fr`).join(' ')};--chart-font-size:${plan.spec.fontSize};--chart-render-font:${(plan.spec.fontSize / innerPageWidth * 100).toFixed(4)}cqi;--arrangement-render-font:${(18 / innerPageWidth * 100).toFixed(4)}cqi;--page-aspect:${plan.spec.pageWidth}/${plan.spec.pageHeight};--page-margin-top:${(plan.spec.margins.top / plan.spec.pageWidth * 100).toFixed(4)}%;--page-margin-right:${(plan.spec.margins.right / plan.spec.pageWidth * 100).toFixed(4)}%;--page-margin-bottom:${(plan.spec.margins.bottom / plan.spec.pageWidth * 100).toFixed(4)}%;--page-margin-left:${(plan.spec.margins.left / plan.spec.pageWidth * 100).toFixed(4)}%;--page-gutter:${(plan.spec.gutter / innerPageWidth * 100).toFixed(4)}cqi">
