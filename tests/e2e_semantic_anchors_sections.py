...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="transposepdf-anchor-") as profile, sync_playwright() as pw:
        ...
        page.locator('.lead-sheet [data-inline-field="section-label"]:visible').first.click(button='right'); page.locator('[data-context-command="duplicate-section-context"]').first.click(); page.wait_for_function('n => window.transposeApp.currentSongs[0].sections.length === n + 1', arg=before)
        page.locator('[data-action="add-section-end"]').click(); page.wait_for_function('n => window.transposeApp.currentSongs[0].sections.length === n + 2', arg=before)
        page.evaluate('() => window.transposeApp.deleteInlineSection(window.transposeApp.currentSongs[0].id, 1)'); page.wait_for_function('n => window.transposeApp.currentSongs[0].sections.length === n + 1', arg=before)
        ...
        page.evaluate("() => window.transposeApp.moveInlineSection(window.transposeApp.currentSongs[0].id, 0, 1)"); page.wait_for_timeout(150)
        checks['section_reorder'] = page.evaluate("id => window.transposeApp.currentSongs[0].sections[1].id===id", drag_id)
